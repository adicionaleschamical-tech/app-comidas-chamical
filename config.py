...
def panel_admin():
    ...
    if tipo_usuario == "admin":
        ...
        with tabs[1]:
            ...
            try:
                df_pedidos = pd.read_csv(f"https://docs.google.com/spreadsheets/d/{st.secrets['sheets']['id_sheet']}/export?format=csv&gid={st.secrets['sheets']['gid_pedidos']}&cb={int(time.time())}")
    ...
    if st.button("Buscar mi pedido", type="primary", use_container_width=True):
        ...
        try:
            with st.spinner("🔍 Buscando tu pedido..."):
                # Usar el ID_SHEET y GID_PEDIDOS de secrets
                try:
                    sheet_id = st.secrets["sheets"]["id_sheet"]
